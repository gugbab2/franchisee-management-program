...
export const BusinessDetailInfo = ({ franBackImg, setFranBackImg }) => {
    ...
    function FranUpdate() {
        ...
        if (document.getElementById('tel').value.length == 0) {
            toast.error('전화 번호를 다시 확인하고 입력하여 주십시오 ', toastdefaultOption);
        }
            ...
            toast.error('변경된 값이 없습니다', toastdefaultOption);
        } else {
            if (document.getElementById('tel').value.length > 8 && document.getElementById('tel').value.length < 15) {
                const dataset = {
                ...
                editAxios(dataset);
                toast.success('가맹점 정보가 수정되었습니다.', toastdefaultOption);
            } else {
                toast.error('전화 번호를 다시 확인하고 입력하여 주십시오', toastdefaultOption);
            }
        }
    ...
    function backImgDel(imageId) {
        instance({
            ...
            toast.success('가맹점 이미지가 삭제되었습니다.', toastdefaultOption);
        });
    }
