...
function Register({ closeRegisterModal }) {
    ...
    const registOnclick = (e) => {
        // console.log(e.target.form[5].value.split('-')[0])
        if (e.target.form[1].value ) {
            if (e.target.form[2].value) {
                if (e.target.form[4].value) {
                        ...
                        if (e.target.form[6].value) {
                            if (registstate.pwchkstate && registstate.emailchkstate
                                ...
                                instance({
                                    ...
                                    closeRegisterModal();
                                    toast.success("회원가입이 완료 되었습니다.", toastDefaultOption);
                                }).catch(function (err) {
                                })
                            } else {
                                
                                console.log(e.target.form[5].value.split('-')[0].length === 4 ? true : false)
                                console.log(e.target.form[5].value.split('-')[1].length === 4 ? true : false)
                                toast.error('전화번호를 확인하세요.', toastDefaultOption);    
                            }
                        } else {
                            toast.error('주소와 우편번호를 확인하세요.', toastDefaultOption);
                        }
                    } else {
                        console.log('이건 뭐지')
                        toast.error('전화번호를 확인하세요.', toastDefaultOption);
                    ...
                    toast.error('이름을 확인하세요.', toastDefaultOption);
                }
            } else {
                toast.error('비밀번호를 확인하세요.', toastDefaultOption);
            }
        } else {
            toast.error('이메일을 확인하세요.', toastDefaultOption);
        }
    ...
    const sendEmail = (e) => {
        ...
        if (registstate.emailchkstate) {
            instance.post(
                ...
                toast.success('인증번호가 전송되었습니다.', toastDefaultOption);
            }).catch((err) => {
                toast.error('이미 회원가입 되어 있습니다.', toastDefaultOption);
            })
        } else {
            toast.error('올바른 이메일 형식을 넣어 주십시오', toastDefaultOption);
        }
    }
    ...
    const chkvalidateEmail = (e) => {
        ...
        if (e.target.form[2].value) {
            instance.get(
                ...
                setActiveTimer(false)
                toast.success('인증이 완료되었습니다.', toastDefaultOption);
            }).catch((err) => {
                toast.error('인증번호를 확인해주세요.', toastDefaultOption);
            })
        } else {
            toast.error('인증번호 값이 비어있습니다.', toastDefaultOption);
        }
    }
