...
function MypageForm() {
    ...
    function UpdateUserInfo(
        ...
        if (userInfo.address.detail === document.getElementById('detailAddress').value && userInfo.phoneNumber === beforeTel.phoneNumber) {
            toast.error('변경된 값이 없습니다', defaultToastOption);
        }
        else if (
            userInfoAddress &&
            ...
            instance({
                ...
                toast.success('유저정보가 수정되었습니다.', defaultToastOption);
            }).catch(function (err) {
                toast.error('전화번호란을 확인해주세요.', defaultToastOption);
                setUserInfo({
                    ...userInfo,
    ...
    useEffect(() => {
        ...
        instance({
            ...
            .then(function (res) {
                if (query) setQueryCount(res.data.searchCount);
                else setSearchCount(res.data.searchCount);
