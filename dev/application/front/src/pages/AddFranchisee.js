...
function AddFranchisee({ inputElement }) {
        ...
        axios
            ...
            .then((response) => {
                if (response.data.match_cnt === 1) {
                    ...
                    toast.error("사업자 번호를 확인해주세요.", toastDefaultOption);
                }
            })
    ...
    const onLoadBackImg = (e) => {
        ...
        instance({
            ...
            if (backImgsrc.length < 10) {
                ...
                toast.error("외관사진은 최대10개까지만 등록가능합니다.", toastDefaultOption);
            }
        });
    };
    ...
    const addFranchiseeFunction = () => {
        ...
        if (time) {
            instance({
                ...
                toast.success("가맹점 등록이 완료되었습니다.", toastDefaultOption);
            ...
            toast.error("영업 시간을 확인해주세요.", toastDefaultOption);
        }
    };
    ...
    const handleNext = () => {
        console.log(franchiseeInfo.franchiseeinput);
                ...
                if (franchiseeInfo.franchiseeinput.phonenumber.length > 8 && franchiseeInfo.franchiseeinput.phonenumber.length < 12 &&
                    ...
                    if (firstImgsrc && firstImgId) {
                        if (
                            ...
                            toast.error("가맹점 주소를 확인해주세요.", toastDefaultOption);
                        }
                    } else {
                        toast.error("가맹점 대표이미지를 확인해주세요.", toastDefaultOption);
                    }
                } else {
                    toast.error("전화번호 란을 확인해 주세요.", toastDefaultOption);

                }
            } else {
                toast.error("가게 명을 확인해주세요.", toastDefaultOption);
            }
        } else {
            inputElement.current.focus();
            toast.error("사업자 번호를 확인해주세요.", toastDefaultOption);
                                    ...
                                    <Row>
                                        ...
                                        >
                                            ...
                                            <InputGroup className="mb-3">
                                                ...
                                                <div id="AddFran-FirstImg">대표 이미지</div>
                                            </InputGroup>
                                        </Form.Group>
                                    </Row>
                                    <Row>
                                    ...
                                    <Row className="AddFran-BackImgRow">
                                        <Col className="AddFran-BackImgCol">
                                            <span className="AddFran--InfoInput-filebox" onChange={onLoadBackImg}>
                                                ...
                                                <input type="file" id="ex_filename" className="upload-hidden" disabled={backImgsrc.length > 9 ? true : false} />
                                            </span>
