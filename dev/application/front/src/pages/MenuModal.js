...
export const AddMenuModal = ({ menuModalshow, setMenuModalshow, businessNumber, setMenuList, menuList, oneMenu, setOneMenu }) => {
    ...
    const menuStateChange = (e) => {
        console.log(e.target.value.replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,"));
    ...
    const MenuAdd = () => {
        ...
        if (menuState.name.length === 0 || menuState.price.length === 0) {
            toast.error('메뉴이름 또는 메뉴가격을 확인해주세요.', toastDefaultOption);
            return;
        } else if (menuState.price > 100000000 || menuState.price < 0) {
            toast.error('메뉴가격은 0~1억사이만 가능합니다.', toastDefaultOption);
            return;
        } else if (menuState.name.length > 10) {
            toast.error('메뉴이름은 10글자를 초과할 수 없습니다.', toastDefaultOption);
            return;
        } else if (menuState.description.length > 150) {
            toast.error('메뉴설명은 150자를 초과할 수 없습니다.', toastDefaultOption);
            return;
        }
        ...
        toast.success('메뉴가 등록되었습니다.', toastDefaultOption);
        setMenuModalshow(false);
    }
    ...
    const MenuEdit = () => {
        ...
        if (oneMenu.name.length === 0 || oneMenu.price.length === 0) {
            toast.error('메뉴이름 또는 메뉴가격을 확인해주세요.', toastDefaultOption);
            return;
        } else if (oneMenu.price > 100000000 || oneMenu.price < 0) {
            toast.error('메뉴가격은 0~1억사이만 가능합니다.', toastDefaultOption);
            return;
        ...
        instance({
            ...
            toast.success('메뉴가 수정되었습니다.', toastDefaultOption);
            setOneMenu('');
        ...
        <>
            <Modal show={menuModalshow} onHide={ModalClose} centered>
                <Modal.Header closeButton>
                    <Modal.Title onClick={(e) => console.log(oneMenu.price)}>
                        {oneMenu ? "메뉴 수정" : '메뉴 추가'}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Row>
                        ...
                        <Col>
                            ...
                            {oneMenu ? (<Form.Control name="name" type="text" placeholder="메뉴이름을 적어주세요." id="menuName" autoFocus value={oneMenu.name} onChange={(e) => { oneMenuStateChange(e) }} />) :
                                (<Form.Control name="name" type="text" placeholder="메뉴이름을 적어주세요." id="menuName" autoFocus value={menuState.name} onChange={(e) => { menuStateChange(e) }} />)}
